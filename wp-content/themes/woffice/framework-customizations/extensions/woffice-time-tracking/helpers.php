    ...
    function woffice_get_tracking_total($tracks) {
        ...
        foreach ($tracks as $index=>$track) {
            ...
            if($track['action'] == 'stop') {
                ...
                // The end timestamp
                $end_float =  woffice_get_time_float($tracks[$index]['timestamp']);

                // The associated start is the one before the stop
                $start_float = woffice_get_time_float($tracks[$index-1]['timestamp']);
        ...
        // Returns a HH:MM
        $formated_time = sprintf('%02d:%02d', (int) $total_time, fmod($total_time, 1) * 60);
