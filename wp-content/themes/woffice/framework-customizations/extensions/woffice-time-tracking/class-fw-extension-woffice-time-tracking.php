...
class FW_Extension_Woffice_Time_Tracking extends FW_Extension {
    ...
    public function _init()
    {
        ...
        // Extra Buttons in the extension's settings
    ...
    public function addScript() {
        ...
        wp_enqueue_script( 'woffice-time-tracking-js', $this->locate_js_URI('woffice-time-tracking'));
    ...
    public function getLog($user_id = 0, $nbr = 10)
    {
        ...
        foreach ($saved_log as $date=>$tracks) {
            $log[$date] = woffice_get_tracking_total($tracks);
        }

        $full_log = $log;

        // We returns the last $nbr entries
        $log_returned = array_slice($full_log, 0, $nbr);
         ...
         */
        return apply_filters('woffice_time_tracking_log', $log_returned, $full_log, $user_id);
    ...
    public function export() {
        ...
        foreach ($users as $user) {
            ...
            foreach ($log as $day=>$hours) {
                fputcsv($output, array($day.','.$hours));
            }
        }
        ...
        wp_redirect(admin_url('admin.php?page=fw-extensions&sub-page=extension&extension=woffice-time-tracking?export=done'));
        die();
