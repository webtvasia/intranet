...
function woffice_is_user_allowed_wiki($post_id = null){
    ...
    if(is_user_logged_in() && !current_user_can('woffice_read_wikies')) {
        return false;
    }
                    ...
                    if (woffice_is_user_allowed_wiki()) {
                        ...
                        if ($wiki_sortbylike) {
                            ...
                            array_push($wiki_array, array(
                                    'string' => '<li class="is-'.get_post_status().'"><a href="' . get_the_permalink() . '" rel="bookmark" data-post-id="'.get_the_ID().'" class="' . $featured_wiki_class . '">' . get_the_title() . $likes_display . '</a></li>',
                                    'likes' => (!empty($like)) ? (int)$like : 0
                                )
                            );
                        } else {
                            $html .= '<li class="is-'.get_post_status().'"><a href="' . get_the_permalink() . '" rel="bookmark" data-post-id="'.get_the_ID().'" class="' . $featured_wiki_class . '">' . get_the_title() . $likes_display . '</a></li>';
                        }
 ...
 */
function display_actions_buttons() {
        ...
        <div class="center" id="wiki-bottom">
            <?php
                ...
                */
                ...
                echo '<a href="javascript:void(0)" class="btn btn-default" id="show-wiki-create"><i class="fa fa-plus-square"></i> ' . $new_wiki_article_button_text . '</a>';
             ?>
