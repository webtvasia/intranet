	...
	class Woffice_Wiki_Display_Manager {
		...
		public function displayCategories( ) {
			...
			foreach ( $categories as $category ) {
                 ...
                 * Column Title
                 */
				$category_title = apply_filters( 'woffice_wiki_category_title', '<i class="fa fa-folder"></i> ' . esc_html( $category->name ) . ' (<span class="wiki-category-count">' . $n_elements . '</span>)', $category->name, $n_elements );
				echo '<div class="heading"><h2><a href="' . get_term_link( $category->slug, 'wiki-category' ) . '">' . $category_title . '</a></h2></div>';
		...
		public function displaySubcategories( $parent ) {
			...
			foreach ($categories as $category_child) {
				...
				if ($this->accordionEnabled) {
					$wiki_subcategory_title = '<li class="sub-category"><span data-toggle="collapse" data-target="#' . $category_child->slug . '" expanded="false" aria-controls="' . $category_child->slug . '">' . esc_html($category_child->name) . ' (' . $n_elements . ')</span>';
			...
			foreach($posts as $post) {
				...
				if ( $this->sortedByLikeInPage ) {
					...
					array_push( $wiki_list['array'], array(
							'string' => '<li class="is-' . $post->post_status . '"><a href="' . get_the_permalink($post) . '" rel="bookmark" class="' . $featured_wiki_class . '" data-post-id="' . $post->ID . '">' . $post->post_title . $likes_display . '</a></li>',
							'likes'  => ( ! empty( $like ) ) ? (int) $like : 0
						)
					);
				} else {
					$wiki_list['string'] .= '<li class="is-' . $post->post_status . '"><a href="' . get_the_permalink($post) . '" rel="bookmark" class="' . $featured_wiki_class . '" data-post-id="' . $post->ID . '">' . $post->post_title . $likes_display . '</a></li>';
				}
		...
		protected function getNumberOfElements( $term_id) {
			...
			$n_elements += count($this->getGroupedPosts($term_id));
